...
def _load_vocab(artifact_id: str) -> List[str]:
    rec = next(r for r in registry.find() if r.get("id") == artifact_id)
    obj = _backend(rec["backend"]).load(rec["data_ref"])
    ...
    raise ValueError("Unsupported vocabulary artifact format – expected list or sklearn vectorizer.")
...
def main(exp: str | None, vocab: str | None, top_n: int, min_cluster_size: int):
