...
def _rec_sort_key(rec: Dict[str, Any]) -> float:
    """Prefer sorting by data_ref mtime; fallback to 0."""
    try:
...
def list_batches(batches: List[Dict[str, Any]], granularity: Optional[str], model_filter: Optional[str]) -> List[int]:
    ...
    for i, r in enumerate(batches):
        ...
        try:
            mtime = Path(r.get("data_ref", "")).stat().st_mtime
        except Exception:
            mtime = 0
...
def _ensure_params_complete(rec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if p.get("granularity") in (None, "", "unknown") or p.get("n_vectors") in (None, "", 0):
        try:
            df = load_df_from_batch(rec)
            if p.get("granularity") in (None, "", "unknown"):
                p["granularity"] = _infer_granularity_from_df(df)
            if p.get("n_vectors") in (None, "", 0):
                p["n_vectors"] = int(len(df))
        except Exception:
            # leave as-is if load fails
            pass
    return p
        ...
        if choice == "q":
            ...
            try:
                umap_model_id = register_model(
                    ...
                    parameters={
                "source_batches": rec.get("batches"),
